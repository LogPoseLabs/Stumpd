...
/**
 * Storage manager for player groups using SharedPreferences
 * Note: This is deprecated in favor of GroupRepository for Room database operations
 */
@Deprecated(
    message = "Use GroupRepository for database-backed group management",
    replaceWith = ReplaceWith("GroupRepository", "com.oreki.stumpd.data.repository.GroupRepository")
)
class PlayerGroupStorageManager(private val context: Context) {
    private val prefs = context.getSharedPreferences(Constants.PREFS_GROUPS, Context.MODE_PRIVATE)
    private val gson = GsonProvider.get()
    private val key = Constants.KEY_GROUPS
    ...
    fun createGroup(
        ...
        defaults: GroupDefaultSettings = GroupDefaultSettings(
            matchSettings = MatchSettings() // Use default MatchSettings
        )
    ): PlayerGroup {
